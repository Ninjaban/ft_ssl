...
static t_bool		ft_ssl_init_b64(t_command **command, void (*function))
{
	int		n;
	...
	if ((*command = malloc(sizeof(t_command) * 11)) == NULL)
		return (FALSE);
	n = 0;
	ft_ssl_init_add_cmd("MODE", &ft_ssl_b64_mode, ninit(0, FALSE, NULL), &((*command)[n++]));
	...
	ft_ssl_init_add_cmd("-d", &ft_b64_decode, ninit(0, FALSE, ft_strsplit("-e", " ")), &((*command)[n++]));
	ft_ssl_init_add_cmd("-e", &ft_b64_encode, ninit(0, FALSE, ft_strsplit("-d", " ")), &((*command)[n++]));

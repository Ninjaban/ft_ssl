...
static void			ft_whirlpool_process_buffer(uint64_t (*L)[8], const uint64_t state[8], const uint64_t K, int n)
{
	(*L)[n] =
			C0[(int)(state[n] >> 56)       ] ^
			...
			C7[(int)(state[(n+1)%8]      ) & 0xff] ^
			K;
}

static void			ft_whirlpool_process_loop(uint64_t (*state)[8], uint64_t K[8], int r)
{
	uint64_t L[8];
...
static void			ft_whirlpool_process_block(t_whirlpool *out_whirlpool, uint64_t (*block)[8])
{
	uint8_t			*buffer;
	int				i;
